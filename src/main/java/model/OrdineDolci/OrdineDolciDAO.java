...
public class OrdineDolciDAO {
    ...
    public boolean updateOrdineDolci(int idOrdine,int idDolce){
        try(Connection conn = ConPool.getConnection()) {
            PreparedStatement ps = conn.prepareStatement("DELETE FROM 'ordine' WHERE 'idOrdine=?'AND 'codice=?';");
            ps.setInt(1,idOrdine);
            ps.setInt(2,idDolce);
            int ritorno=ps.executeUpdate();
            if (ritorno==2) return false;
